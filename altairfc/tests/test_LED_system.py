...
class Ldac:
    ...
    def __init__(self, pin, enabled):
        ...
        print(
            f"[INFO] LDAC (BCM {pin}) held LOW — DAC writes will update VOUT immediately"
        )
...
def ads1115_one_shot_read_mux(
    bus, addr, mux_bits, gain=2, data_rate=_ADS1115_DR_128SPS
):
...
def main():
    ...
    parser.add_argument(
        "--bus", default="/dev/i2c-1", help="Shared I2C device node for both chips"
    )
    parser.add_argument(
        "--mcp4728-addr", default=hex(MCP4728_ADDR), help="MCP4728 I2C address"
    )
    parser.add_argument(
        "--ads1115-addr", default=hex(ADS1115_ADDR), help="ADS1115 I2C address"
    )
    parser.add_argument(
        "--ads1115-gain",
        type=int,
        choices=sorted(_ADS1115_GAIN_FSR),
        default=2,
    ...
    try:
        mcp4728_multi_write_channel(
            bus, mcp4728_addr, args.channel, args.code, vref_vdd=True, gain=1
        )
    except OSError as e:
    ...
    if args.channel in (2, 3):
        print(
            f"[WARN] --channel {args.channel} reads AIN{args.channel} single-ended, which shares "
    ...
    print(
        f"[OK] MCP4728 channel {ch_letter} set to code {args.code}/{MCP4728_MAX_CODE}"
    )
    print(
        f"Monitoring ADS1115 at 0x{ads1115_addr:02X}: thermistor bridge (AIN2-AIN3, differential) "
    ...
    try:
        while True:
            try:
                therm_code = ads1115_one_shot_read_mux(
                    bus, ads1115_addr, _ADS1115_MUX_DIFF_2_3, gain=args.therm_gain
                )
            ...
            try:
                ...
                led_code = ads1115_one_shot_read_mux(
            ...
            print(
                f"[{ts}] CH{ch_letter} code={args.code:4d}  "
        ...
        try:
            mcp4728_multi_write_channel(
                bus, mcp4728_addr, args.channel, 0, vref_vdd=True, gain=1
            )
